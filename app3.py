...
if __name__ == '__main__':
    ...
    # Checkout the correct Git branch when the server starts.
    try:
        print("Checking out the 'update-rtr1-config' branch...")
        subprocess.run(['git', 'checkout', 'update-rtr1-config'], check=True, cwd=SCRIPT_DIR)
        print("Branch checkout successful.")
    except subprocess.CalledProcessError as e:
        print(f"Error: Git branch checkout failed. Ensure the 'update-rtr1-config' branch exists. Error: {e}")
        # Exit if checkout fails to prevent further errors
        exit(1)
